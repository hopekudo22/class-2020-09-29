...
```{r q1}
d_2019 <- read_excel("downloaded_data//class_enrollment_summary_by_term_9-22-2020.xlsx", 
  skip = 3) %>% 
    clean_names() %>%
    rename_with(~ str_replace(.x, "course_", "")) %>%
    select(id, title, name, departmnet, u_grad) %>%
    filter(u_grad > 10)
...
d_2018 <- read_enrollment("raw_data/class_enrollment_summary_by_term_10.24.18.xlsx", x = 2)
d_2017 <- read_enrollment("raw_data/class_enrollment_summary_by_term_10.20.17.xlsx")
d_2016 <- read_enrollment("raw_data/class_enrollment_summary_by_term_fall_2016.xlsx", x = 0)
...
total_data <- bind_rows(d_2019, d_2018, d_2017, d_2016, d_2015, id = NULL)
